...
class ApplicationBloc with ChangeNotifier {
  ...
  //   String stationName = (await _placesService.getPlaceFromCoordinates(station.lat, station.lng, "Santander Cycles: ${station.name}")).name;
  //   return stationName;
  // }
  ...
  Future<int> _getDurationFromToStation(Station startStation, Station endStation) async{
    ...
    print("??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????START PLACE ID: " + startStation.place.placeId);
    print("??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????END PLACE ID: " + endStation.place.placeId);
      ...
      if (await _getDurationFromToStation(curStation, endStation) <= 25) {
        ...
        for (int i = 0; i < nearbyStations.length; i++) {
          if(nearbyStations[i].place == const Place.placeNotFound()){
            Place place = await PlacesService().getPlaceFromCoordinates(nearbyStations[i].lat, nearbyStations[i].lng, "Santander Cycles: ${nearbyStations[i].name}");
          ...
          if ((await _getDurationFromToStation(curStation, nearbyStations[i])) <= 25) {
            ...
            if(curStation.place == const Place.placeNotFound()){
              Place place = await PlacesService().getPlaceFromCoordinates(curStation.lat, curStation.lng, "Santander Cycles: ${curStation.name}");
              curStation.place = place;
            }
            setLocationMarker(curStation.place, i + 69);
            //TODO: Add to pathway so markers are removed
            break;
          }
        }
      }
