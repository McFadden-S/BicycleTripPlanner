...
void main() {
  ...
  testWidgets("HomeWidgets has a stack", (WidgetTester tester) async {
    ...
    final BuildContext context = tester.element(find.byType(HomeWidgets));
  ...
  testWidgets("HomeWidgets has a CurrentLocationButton",
  ...
  testWidgets("HomeWidgets has a navigateToRoutePlanningScreenButton",
      (WidgetTester tester) async {
    await pumpWidget(tester, MaterialApp(home: Material(child: HomeWidgets())));
    expect(
        find.byKey(Key("navigateToRoutePlanningScreenButton")), findsOneWidget);
  });

  testWidgets("HomeWidgets has a bottom StationBar",
      (WidgetTester tester) async {
    await pumpWidget(tester, MaterialApp(home: Material(child: HomeWidgets())));
  ...
  testWidgets('settings button navigate user to settings screen',
    ...
    expect(
        find.byKey(Key("navigateToRoutePlanningScreenButton")), findsNothing);
  });
  ...
  testWidgets(
      'GroupSizeSelector has correct initial value and behaves correctly',
      (WidgetTester tester) async {
    await pumpWidget(
        tester,
        MaterialApp(
    ...
    expect(
        (tester.widget(find.byKey(Key("groupSizeSelector"))) as DropdownButton)
            .value,
        equals(1));
    ...
    for (int i = 1; i <= 10; i++) {
      expect(
          find.byKey(Key(i.toString()), skipOffstage: false), findsOneWidget);
    }
  });
}
